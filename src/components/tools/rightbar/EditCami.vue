    ...
    <div>
      <div>
        <div class="form-group"><label><span>所属商品</span></label>
          <div  @click="setSelectorPos" ref="selector" :class="selected ? 'ant-select ant-select-open ant-select-focused ant-select-enabled form-group' : 'ant-select ant-select-enabled form-group'" style="width: 100%;">
            <div class="ant-select-selection ant-select-selection--single" role="combobox" aria-autocomplete="list" aria-haspopup="true"
                 aria-controls="63c5b191-6ada-4935-d770-5a29756e6295" aria-expandant-select-selection__rendereded="false" tabindex="0">
              <div class="ant-select-selection__rendered">
                <div class="ant-select-selection-selected-value" title="无" style="display: block; opacity: 1;">{{getName(cami.goodsId)}}</div>
              </div>
              <span class="ant-select-arrow" unselectable="on" style="goods-select: none;"><i aria-label="图标: down"
        ...
        <div class="form-group"><label for="example-text-input-alt">所属订单</label><input
            placeholder="如需修改密码请输入" type="text" class="ant-input" v-model="cami.outTradeNo"></div>
        <div class="form-group"><label for="example-text-input-alt">卡密</label>
          <div><textarea rows="4" placeholder="请在这里记录.." class="ant-input" v-model=cami.cami></textarea></div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      this.isSubmit = true
      if (this.cami.status === '' || this.cami.goodsId === '' || this.cami.content === '') {
        let tip = {
          tipTitle: "请求失败",
          tipMsg: "请输入有效邮箱",
          type: false,
      ...
      const {data: res} = await this.$http.put('item/' + this.id, {
    ...
    clear(){
      this.cami = {
        status: '', goodsId: '',outTradeNo:'', content: ''
      }
    },
  },
  mounted() {
