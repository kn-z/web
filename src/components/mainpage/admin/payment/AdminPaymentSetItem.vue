...
export default {
    ...
    methods: {
        changeFocus() {
            this.$bus.$emit(this.pageDict[this.dictKey].switchItemFocus, this.payment.id)
        },
        clearFocus() {
            this.$bus.$emit(this.pageDict[this.dictKey].switchItemFocus, null)
        },
        edit() {
            this.$bus.$emit('RBReload')
            this.$bus.$emit(this.pageDict[this.dictKey].reloadRBName, this.payment.id)
            this.$bus.$emit(this.pageDict[this.dictKey].getRBTName, 'E')
            this.$bus.$emit('openRB', this.dictKey)
        }
    },
    mounted() {
        this.$bus.$on(this.pageDict[this.dictKey].switchSetFocus, (data) => {
            this.focusTo = data
        })
    }
}
