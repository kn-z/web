...
export default  {
  ...
  methods: {
    changeFocus() {
      this.$bus.$emit('switchUSFocus', this.user.id)
    },
    clearFocus() {
      this.$bus.$emit('switchUSFocus', null)
    },
    edit(){
      this.$bus.$emit('RBReload')
      this.$bus.$emit('openRB',this.dictKey)
      this.$bus.$emit(this.pageDict[this.dictKey].reloadRBName,this.user.id)
    }
  },
  mounted() {
    this.$bus.$on('switchUSIFocus',(data)=>{
      this.focusTo = data
    })
  }
}
